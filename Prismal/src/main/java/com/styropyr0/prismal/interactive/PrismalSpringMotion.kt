...
/**
 * Spring-based drag controller for glass sliders, toggles, and tab indicators.
 *
 * Manages value animation, press scale, and velocity tracking. Attach [modifier] to
 * the draggable surface and read [value], [pressProgress], and [scaleX]/[scaleY] in
 * glass effect blocks.
 *
 * @param initialValue Starting value within [valueRange].
 * @param initialScale Resting scale factor applied while not pressed.
 * @param pressedScale Scale factor while the user is dragging.
 */
class PrismalSpringMotion(
    ...
    val onDragStarted: PrismalSpringMotion.(position: Offset) -> Unit,
    val onDragStopped: PrismalSpringMotion.() -> Unit,
    val onDrag: PrismalSpringMotion.(size: IntSize, dragAmount: Offset) -> Unit,
) {
